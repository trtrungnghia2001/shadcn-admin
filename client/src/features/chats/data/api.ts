...
export const getUsersApi = async (query = "") => {
  return (
    await axiosInstance.get<ApiResponse<ChatUserType[]>>(`/chat/users?` + query)
  ).data;
};
...
export const getMessageApi = async (receiverId: string, query = "") => {
  return (
    await axiosInstance.get<ApiResponse<ChatMessageType[]>>(
      `/chat/users/message/` + receiverId + "?" + query
    )
  ).data;
};
