...
import autoCacheMiddleware from "../middlewares/cache.js";

const chatRouter = express.Router();

chatRouter.post(
  "/send",
  autoCacheMiddleware("chat_history"),
  async (req, res, next) => {
    try {
      const body = req.body;
...
chatRouter.get(
  "/users",
  autoCacheMiddleware("chat_users", 10),
  async (req, res, next) => {
    try {
      const userId = req.user._id;
...
chatRouter.get(
  "/users/message/:id",
  autoCacheMiddleware("chat_history", 60),
  async (req, res, next) => {
    try {
